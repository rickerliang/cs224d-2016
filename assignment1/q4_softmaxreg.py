...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    #print 'sentence.shape', len(sentence)
    #print 'sentVector.shape', sentVector.shape
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    grad_ce_to_softmax_input = prob - labels_onehot
    grad_ce_to_weights = np.einsum('ij,ik->ikj', grad_ce_to_softmax_input, features)
    #print 'grad_ce_to_weights.shape', grad_ce_to_weights.shape
    grad_ce_to_weights = np.sum(grad_ce_to_weights, axis=0) / N
    grad_regularization_to_weights = regularization * weights
    grad = grad_ce_to_weights + grad_regularization_to_weights
    ### END YOUR CODE
